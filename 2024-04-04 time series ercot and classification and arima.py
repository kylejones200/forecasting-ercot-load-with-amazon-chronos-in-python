...
def suppress_warnings() -> None:
    ...
    mean_absolute_error(y_reg_test, y_reg_pred)
    ...
    if best_model is not None:
        ...
        last_value = series_train["values"].iloc[
            -1
        ]  # Last observed value in training data
        y_reg_pred_arima = np.cumsum(y_reg_pred_arima_diff) + last_value
...
def suppress_warnings_2() -> None:
    ...
    df["values"].iloc[-hold_out_hours:]  # Actual values for hold-out period
